...
template <class T>
void BST<T> :: insert1(string value,T k,string flag){
	if(flag=="right")
    {
   		root = mergeintoright(value,k,root,flag);
    }
    else
    {
   		root = mergeintoleft(value,k,root,flag);
    }
}
template <class T>
node<T>* BST<T> :: mergeintoright(string value,T k, node<T> *p,string flag)
{ 
	if(p == NULL)
	{
		node<T>* insert = new node<T>(k,value);
		return insert;
	}
	else
	{
		p->right=mergeintoright(value,k,p->right,flag);
	}
    return p;
}

template <class T>
node<T>* BST<T> :: mergeintoleft(string value,T k, node<T> *p,string flag)
{ 
	if(p == NULL)
	{
		node<T>* insert = new node<T>(k,value);
	...
	{
		p->left=mergeintoleft(value,k,p->left,flag);
	}
    return p;
}
